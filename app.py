...
def generate_images_using_huggingface_diffusers(text):
    # Load the model on CPU instead of CUDA (GPU)
    pipe = StableDiffusionPipeline.from_pretrained("runwayml/stable-diffusion-v1-5", torch_dtype=torch.float32)  # Use float32 for CPU
    pipe = pipe.to("cpu")  # Ensure the model uses CPU
    prompt = text
